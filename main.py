        ...
        "threads": [
            {
                ...
                "summary": t.summary if include_content else "{"karma_required": 10, "message": "🔒 Locked — earn karma to unlock"}",
                "url": t.url if include_content else "{"karma_required": 10, "message": "🔒 Locked — earn karma to unlock"}",
            }
            for t in threads
        ],
...
async def signal_feed(
    ...
    if agent.karma < 10:
        raise HTTPException(status_code=403, detail={"error": "karma_required", "current": agent.karma, "needed": 10, "message": "🔒 Not enough karma! Curate some Moltbook posts to level up."})
